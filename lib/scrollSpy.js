...
const getScrollDirection = (scrollTop, oldScrollTop, direction) => {
  ...
  return oldScrollTop - scrollTop < 0 ? 'down' : 'up'
}

let defferedScrollDirectionChange = false
const scrollDirectionDidchange = (scrollDirection, oldScrollDirection, scrollTop, scrollHeight) => {
  ...
  if (scrollTop <= 0) {
    defferedScrollDirectionChange = true
    return false
  } else if (scrollTop >= scrollHeight) {
    defferedScrollDirectionChange = true
    return false
  } else if (defferedScrollDirectionChange) {
    defferedScrollDirectionChange = false
    return true
  }
